...
public class APINameBuilder implements JPAEdmNameBuilder {
	...
	public String buildEntitySetName(final String entityTypeName) {
    	...
    	eSetName = "CompetativeAnalysis".equals(entityTypeName) ? "CompetativeAnalysis" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "DataAnalysis".equals(entityTypeName) ? "DataAnalysis" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "Survey".equals(entityTypeName) ? "Surveys" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "AdvertisingAgency".equals(entityTypeName) ? "AdvertisingAgencies" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "MediaPlanAndBuying".equals(entityTypeName) ? "MediaPlanAndBuyings" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "BrandPerception".equals(entityTypeName) ? "BrandPerceptions" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "FocusGroup".equals(entityTypeName) ? "FocusGroups" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "MarketResearch".equals(entityTypeName) ? "MarketResearches" : defaultNameBuilder.buildEntitySetName(entityTypeName);
    	eSetName = "TargetMarketIdentification".equals(entityTypeName) ? "TargetMarketIdentifications" : defaultNameBuilder.buildEntitySetName(entityTypeName);
        return eSetName;
    }
